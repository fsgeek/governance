...
% NAMING the nearest prior art and stating the RESIDUAL, rather than dodging it.
% Citations verified against primary sources. Tony to accept / reword / reject ---
% this is prose, so it is your call, not mine to commit. Proposed insertion:
% here, as a new subsubsection or folded into the Goodhart subsection above.
%
...
% of substance. Each names a version of the pattern this section is built on:
% an artifact standing where evidence of substance is expected. We do not claim
% the phenomenon is new. What silence-manufacture adds is a distinction those
% frameworks do not draw. Decoupling is an \emph{act}: an organization could
% couple structure to activity and, for reasons of legitimacy, cost, or capture,
% chooses not to; the ceremonial artifact is a substitution someone could have
% avoided. Silence-manufacture, in the architectural cases this paper concerns,
% is frequently not a choice at all. Where the binding between an explanation and
% a model's reasoning is architecturally unavailable, or where the silence is
% \emph{legally mandated} --- as under the BSA/AML tipping-off prohibition, which
% forbids the institution from disclosing the very fact whose absence is then read
% as all-clear --- there is no coupled alternative the institution declined to
% produce. The decoupling frame, which reads every ceremonial artifact as a
% substitution that could have been otherwise, cannot make this cut, because it
% has no account of \emph{when} the substantive evidence is architecturally or
% legally available to begin with. That distinction --- between a silence an
% institution manufactures and a silence its architecture or its statute produces
% for it --- is what the term is for, and it is the boundary at which the
% culpability the audit-society critique presumes may or may not attach.
% ============================================================================
