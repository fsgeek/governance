...
%
% Status: integrated draft. Silence-manufacture promoted to organizing concept
% per concentrated-placement decision; three diagnostic techniques framed as
% silence-breaking instruments. Architect-primary register preserved
% (examiner-primary draft retained for Paper 3). Mapping to three earned
% positions includes cross-domain empirical confirmation from ML
% interpretability and alignment research. Will be reworked.
%
% Citations needed: Goodhart 1975 and Strathern 1997 for the dual relationship;
% Lanham et al. 2023 and Turpin et al. 2023 for chain-of-thought unfaithfulness;
% Sharma et al. on sycophancy, Perez et al. on RLHF reward hacking, and the
% broader sycophancy literature for the position-3 mapping; benchmark gaming
% and eval-set contamination literature (Goodhart-on-safety-evals) for the
% position-1 mapping; design-methodology references where available; Brooks on
% conceptual integrity and Wing on computational thinking applied to system
% design if useful.

\section{Diagnostic Techniques}
\label{sec:diagnostics}

The empty-chair frame produces interpretive structure (Section~\ref{sec:frame-as-method}) and a structural taxonomy of AI applications by verification-regime availability (Section~\ref{sec:verification-regimes}). The three positions earned by the body sections (that governance which does not induce auditable constraint is theater, that post-hoc explanation is structurally inadequate as a verification regime in adversarial contexts, and that human review of explanations is not verification of reasoning) appear at first as three separate critiques of three different practices. They are not. They are three sites where the same structural failure mode operates, and naming the underlying mode is what makes the categoricals cohere into a structural argument rather than a list of related concerns. We name the mode \emph{silence-manufacture} and develop it in this section as the organizing concept beneath the empty-chair frame. The three diagnostic techniques that follow are \emph{silence-breaking instruments} against silence-manufactured artifacts. Their value is not in producing better metrics; their value is in making audible what the architecture has architecturally suppressed.
...
% ============================================================================
% CANDIDATE PARAGRAPH (staged 2026-07-07, NOT yet in the paper body) --- closes
% rikuy MAJOR ADV-C-003 / ADV-A-008 and the author's own line-10 TODO ("Goodhart
% 1975 and Strathern 1997 for the dual relationship"). Answers the reviewer's
% "silence-manufacture may be re-badged decoupling / audit-society" objection by
% NAMING the nearest prior art and conceding the phenomenon isn't new, then
% marking the one distinction decoupling clearly can't make: legally/architecturally
% MANDATED silence, where no coupled alternative was declined.
%
% TRIMMED (was 9 sentences): the longer draft staked the act-vs-fact "residual"
% on the subtractive-operator theorem, which is adversary-narrowed/fragile and its
% receipts are argument-grade notes, not in the deposit --- too weak to stake in
% the paper. Cut to a defensible concession + the BSA/AML mandated-silence case,
% which stands on the paper's own worked example and needs no fragile import.
% The dropped last sentence ("culpability the audit-society critique presumes")
% opened a philosophical front the position paper doesn't otherwise carry.
%
% Tony: voice check only --- the argument is mine to stand behind; whether the
% prose reads as yours is the call the repo convention reserves to you.
% Insertion: here, as its own subsubsection after the Goodhart one.
%
% \subsubsection{Relationship to decoupling and the audit society}
% \label{sec:diagnostics:silence-manufacture:decoupling}
%
% Silence-manufacture has close neighbors in organizational sociology, and the
% claim earns nothing by ignoring them. Meyer and Rowan's decoupling
% \parencite{meyer1977} describes formal structures adopted ceremonially for
% legitimacy and held apart from operational activity; Power's audit society
% \parencite{power1997} and the audit-culture literature \parencite{strathern1997}
% describe the spread of ritualized verification that certifies process in place
% of substance. The phenomenon --- an artifact standing where evidence of substance
% is expected --- is not new, and we do not claim it is. What the architectural
% cases in this paper add is one distinction those frameworks do not draw: decoupling
% presumes an artifact an organization \emph{could} have coupled to activity and did
% not, whereas the silence here is frequently not a declined choice but a produced
% one --- as under the BSA/AML tipping-off prohibition, which \emph{legally forbids}
% the institution from disclosing the very fact whose absence is then read as an
% all-clear. Where the substantive evidence is architecturally or legally unavailable
% to begin with, there is no coupled alternative the institution declined, and the
% decoupling reading --- every ceremonial artifact as a substitution that could have
% been otherwise --- does not reach the case.
% ============================================================================
...
The architect's question to themselves at this stage: \emph{what does my pattern of attestation reveal about my posture toward empty-chair representation, and is that what I intend it to reveal?}
...
The architect's question to themselves at this stage: \emph{for every silence my system produces, am I confident the silence reflects the absence of objection rather than the suppression of objection by the system itself?}
