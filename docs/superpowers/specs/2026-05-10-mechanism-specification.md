...
**Resolution (2026-05-11 via OD-13):** Path 2 (collector standardization) adopted. `wedge/collectors/lendingclub.py:derive_label`, `wedge/collectors/hmda.py:derive_label`, and `wedge/collectors/fanniemae.py:derive_origination_and_label` now emit `label=1 ⇔ grant`. Structural cascade: `wedge/attribution.py` constants flipped (`GRANT_CLASS_IDX = 1`, `DENY_CLASS_IDX = 0`); `wedge/models.py:emit_for_case` proba-index mapping flipped so `T = P(label=1)`; `wedge/tests/fixtures.py:tiny_separable_dataset` rule inverted to produce `label=1` for low-risk cases. The construction-boundary adapter named in the V1 default is no longer needed and was never implemented in code. Prior empirical findings (cliff structure, T-silent-all / F-silent-all cases) are preserved structurally but their interpretive labels swap: a case formerly "T-silent-all" (no member articulates grant-supportive evidence) is now F-silent-all under the new convention, since the *semantic* meaning of T (confidence in grant) is preserved while the *label* that maps to it has flipped. The 2026-05-09 SHAP non-inferiority result and prior cliff-synthesis memos cite labels under the old convention; re-interpretation in the new convention is a documentation task tracked separately.
