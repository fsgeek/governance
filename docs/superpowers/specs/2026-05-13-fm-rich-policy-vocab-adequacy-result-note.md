...
**rb05 is the most interesting non-verdict cell** (`median_pairwise_spearman ≈ 0.998` → near-identical predictions across members → NOT plural under the pre-reg's plurality criterion → does NOT count toward the strict P1 verdict). The 4 distinct used-feature-sets are `{dti, property_state}`, `{dti, FTHB, property_state}`, `{dti, loan_purpose, property_state}`, `{dti, property_state, servicer_name}` — every member uses (`dti`, `property_state`). R²_named = 0.208 looks gap-like, ΔR²_ext = 0.753 is huge — but the variant-B closure is **0.208 → 0.688 using DTI (0.71 importance) + FTHB (0.20)** — that is, *named features carry it when the geography feature is prohibited*. The depth-3 named-only explainer (variant A) couldn't see the structure because the variant-A band keys on the DTI-within-state *interaction*: split on property_state first (a coarse partition), then on DTI within state-branches; the named-only depth-3 tree without property_state can't represent that two-step structure and gets only R²=0.208. Variant-B's reorganized band, forbidden from property_state, finds a different decomposition where DTI+FTHB carry the load — and the named-only explainer captures it. **This is the pre-reg's predicted "rung 2.5" informative failure mode for P4: a named-feature carrier (DTI) that the depth-3 named explainer misses because the variant-A band keys on a geography*named interaction the explainer's functional form can't represent.** It is more interesting than a textbook rung-3b cell because it says *the named vocabulary is sufficient, but the deployed explainer's depth-3 functional form is not* — a finding about the *artifact*, not the policy.
...
**C-scrambled on gap cells:** band size **7 distinct members on rb00** (vs 80 on 2018Q1 prime cells, vs 38 distinct used-feature-sets on the *real* variant-B band of rb00). Reversed monotonicity bites HARD where geography dominates `d` — i.e. the documented monotonicity on FICO/DTI/LTV/CLTV/MI IS binding on the gap cells, even though those cells don't *use* the monotone features. (The data is monotone enough in those features that violating the constraint excludes most ε-good models.) This is the dead-letter sibling pre-reg surfacing inverted: monotonicity is *slack on the cells the policy describes well* and *binding on the cells the policy describes poorly*.
...
| P4 — variant B closes gap | 0.8 | N/A | HIT on verdict-cells (rb00 0.04→0.77, rb09 0.02→0.74); rb05 (non-verdict, not plural) closes 0.21→0.69 via DTI+FTHB — **the rung-2.5 informative-failure case the pre-reg anticipated** | partial — rb09 0.076→0.264, *still gap*; variant B allows `original_upb` which carries half the disagreement | **partial HIT + 1 confirmed rung-2.5** — B closes when carrier is purely geography/lender; fails when carrier includes loan-size; rb05's named-feature carrier was hidden by the depth-3 explainer's functional form, not by the policy vocabulary |
