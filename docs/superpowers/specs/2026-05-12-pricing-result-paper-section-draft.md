...
**Open beats:** FM cross-regime arm DONE for a 6-vintage regime-spanning sample (2006/2008/2012/2016/2018/2022 Q1, ~5 rate environments) — every rate-band on every vintage is Cat 2 (pricing), FICO-dominated, LTV's role tracks regime stress; see `2026-05-12-fm-cross-regime-pricing-note.md`. Remaining: the LLPA-grid-cells-as-tiers refinement (option C — audits the documented pricing artifact directly), the protected-class-proxy follow-up (converts "coarse" into "disparate impact" or rules it out), option (2) calibration-gap mechanism, and figures (TODO markers below).
...
The cross-regime test is now in hand. The LC result's binding caveat (§5) is that two adjacent 2015 quarters share a rate environment. Fannie Mae's Single-Family Loan Performance data spans 2000–2022 in a unified layout and exposes a continuous origination rate; running the same mechanism on six FM acquisition quarters spanning pre-crisis boom (2006Q1), crisis trough (2008Q1), post-crisis recovery (2012Q1), low-rate era (2016Q1, 2018Q1), and rate-spike onset (2022Q1) — with the tier = `orig_interest_rate` deciled within each vintage — yields **every rate-band on every vintage is Cat 2 (pricing)**, FICO-dominated, across a 28x range of base default rates (0.20% in 2012Q1 to 5.76% in 2008Q1). LTV's prominence in the recovered structure tracks regime stress (major in 2006/2008/2022, minor in 2012/2016/2018) — a clean regime-dependence. The caveat (developed in `2026-05-12-fm-cross-regime-pricing-note.md` §6): FM's rate is set off the LLPA grid (FICO×CLTV cells, intentionally coarse), so "FICO predicts default within a rate-band" is partly the consequence of the grid's bin coarseness — this makes the FM result a **robustness/scaling** demonstration (the mechanism works across regimes and at FM's 200k–700k-loan scale), not a second independent "FM mispriced" headline alongside the LC DTI finding. What it does add: the within-band coarseness is regime-robust (a protected-class-proxy follow-up will not be regime-confounded), and the grid's bins conflate ~2x+ within-band risk differences along policy-named factors in every regime ("by design" ≠ "fine"). The eventual paper's spine is verdict-null → LC pricing-reversal → FM cross-regime robustness; the open refinement is the LLPA-grid-cells-as-tiers version (option C), which audits the documented pricing artifact directly and yields the cleaner "the grid is too coarse" finding free of the rate-as-noisy-proxy concern.
