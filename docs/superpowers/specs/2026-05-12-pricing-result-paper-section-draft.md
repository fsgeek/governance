...
**Open beats:** FM cross-regime arm (FM 2018Q1 pilot done — rate-band tier construction validated, all 9 bands Cat 2 (pricing), FICO-dominated; cross-regime still pending additional FM quarters); option (2) calibration-gap mechanism; figures (see TODO markers).
...
The open beat is the cross-regime test. The LC result's binding caveat (§5) is that two adjacent 2015 quarters share a rate environment. Fannie Mae's Single-Family Loan Performance data spans decades and multiple regimes (pre-crisis, crisis, post-crisis, post-QM) and exposes a continuous origination rate; the pricing-version question there is "for a (rate-band, vintage) cohort, is realized default consistent with the rate under the policy vocabulary, or can a policy-expressible factor stratify the cohort?" — with rate-band × vintage playing the role of `sub_grade`. A single-vintage FM pilot (2018Q1) confirms the rate-band tier construction works and the mechanism scales to FM's ~400k-loan size: all 9 rate-bands are Cat 2 (pricing), dominated by FICO — though with the caveat that FM's rate is set off the LLPA grid (FICO×CLTV cells, intentionally coarse), so "residual FICO predicts default within a rate-band" is partly the consequence of the grid's bin coarseness rather than a pricing error, and a protected-class-proxy follow-up would be needed to call it a governance defect. The eventual paper's spine is verdict-null → LC pricing-reversal → FM cross-regime confirmation (or its absence). The first two beats are in hand; the FM pilot is the pre-flight check; the multi-vintage cross-regime FM run is the work that makes the result regulator-credible rather than illustrative.
