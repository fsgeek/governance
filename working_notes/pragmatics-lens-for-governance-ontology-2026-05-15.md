...
The standard taxonomy has five families (top-down formal, bottom-up data-driven, LLM-assisted, pattern-based, hybrid/CQ-driven). All five are semantics-centric. None types pragmatic context as a construction primitive. The relevant question for each: *how* does it smuggle pragmatics in unlabeled?
...
**Cross-cutting finding from this lens.** All five families face the same four-fold gap, viewed from four angles: incompleteness representation, temporal/regulatory versioning as semantic (not metadata), contested-term representation, construction-time provenance bound to axioms. These are not four separate problems. They are one problem — *the pragmatic layer isn't typed* — seen through four operational windows.
...
- Alignment: AML, LogMap, BERTMap, the OAEI-LLM line — given two ontologies, produce a mapping. The 2025 OAEI Bio-ML had no single winner across tasks: BioGITOM led on two of five semi-supervised tasks (NCIT–DOID, OMIM–ORDO); BERTMap and LogMap-LLM each led on one of the remaining tasks.
- Structural metrics: OntoMetrics (a battery of schema/graph/class-axiom metrics), OOPS! (41-pitfall catalog), OQuaRE.
...
**FRO (Financial Regulation Ontology) — audited.** Single-maintainer (Jurgen Ziemer / Jayzed Data Models); imports both FIBO (entities) and LKIF (legal expressions); loads CFR Title 12 (Banking) and Title 17 (Investment Adviser Act) plus USC Titles 12 and 15. The worked example is whether an investment fund must register with the SEC. Dual-licensed GPL-3.0 + MIT (declared via `dct:license` in each `.ttl` file, though not on the public site). FRO *does* model the interpretation act structurally: `jc-core:Judicial_Interpretation` types judicial readings; `lkif-proc:created_by` links rules to their rulemaking events; `fro-leg-ref:DocumentEdition` represents annual-edition versions of CFR/USC. What FRO does **not** type: provisional rules, contested interpretations, deliberately-unspecified regions, interpretation-validity periods, or "the regulator has not yet ruled on X." Interpretation acts are represented; the *pragmatic context of the interpretation* (interpreted-by-whom, under-which-edition, with-what-validity-window, disputed-by-whom) is not. **FRO is the closest published artifact to a policy-codification ontology — comparable, not competing infrastructure** — and its absences are exactly the pragmatic-layer absences the lens names.
...
- *Kumar, Hines & Dickerson* (AIES 2022, arXiv 2210.02516), "Equalizing Credit Opportunity in Algorithms: Aligning Algorithmic Fairness Research with U.S. Fair Lending Regulation." The cleanest articulation that the fairness-ML literature is largely disconnected from how Reg B disparate-impact analysis actually runs. **Worth a careful read for Paper 1 framing — same diagnosis (the layer that does the work isn't where the field is looking), different mechanism (here: protected-class proxies; in the project: pragmatic context).**
- *SR 26-2* (Fed, April 17, 2026), superseding SR 11-7. The new guidance introduces materiality calibration (model materiality = exposure × purpose) and requires the model inventory to "support effective model risk management at the individual and aggregate levels." Critically, the letter does **not** specify the *structural form* of the inventory — the Model Inventory section is one paragraph asking for "a comprehensive set of information" and "sufficient information to understand model risks." **The regulator opened the window without specifying its shape.** The unspecified shape *is* the pragmatic-layer opportunity: a knowledge-graph-shaped inventory is one plausible operationalization of the aggregate-risk requirement, but the letter leaves the choice of representation, queryability, and version-of-deployed-policy semantics to the supervised entity. That gap is where governance-ontology infrastructure can land — or where vendor lock-in to a flat-tabular form will quietly happen first.
- *EU AI Act* (Regulation 2024/1689) classifies credit scoring as high-risk AI (Annex III) and requires transparency and human-oversight infrastructure for high-risk systems (Articles 13–14). The specific "meaningful information about the logic involved" explainability language traces to **GDPR Article 15(1)(h)** and was reinforced by **CJEU Case C-203/22** (February 2025); together with the AI Act's high-risk classification, these constitute the European-parallel requirement. The AI Act itself does not use the "meaningful information" phrasing — the explainability anchor remains GDPR/CJEU.

---

## 5. The lens applied to the project's own work

Recent project work generates empirical contact with the pragmatic layer without consistently naming it. The lens makes this visible.
...
**HMDA trimodal-replication MISS (OTS-stamped 2026-05-14).** P-scorecard 0/5 HITs (with 3 N/A). The pre-registered prediction that FM-substrate trimodal saturation phases would generalize to HMDA-RI 2022 fell to two structural reasons: HMDA reorganization decouples from carrier saturation; and the FM-calibrated R²≥0.30 adequacy threshold transferred as a floor on HMDA, structurally suppressing verdict_differs. *The trimodal claim tightens to "FM-substrate-validated," not universal.* This is substrate-indexicality. The same machinery does not carry across substrates without explicit substrate-context typing.

**The pre-registration failure pattern itself.** Across recent pre-regs, the dominant failure mode is *uniformity-bet → falsification → tightening-to-indexed-claim*. The pattern is consistent enough that the project memory now flags it as a structural observation: pre-registered predictions assume uniformity; reality is indexed; the failure pattern itself motivates indexicality as a load-bearing typing axis. **The pre-reg failure record is, under this lens, the project's empirical case for naming the layer.**

**H2 — frame-evocation falsification (OTS-stamped 2026-05-15).** Three discriminators tested on the FM #12 corpus: R²-proximity (current pipeline proxy), subset-membership (current schema primitive `mandatory_features` as enforcement), frame-coherence (the d-signal's max univariate |ρ| on named features). The load-bearing pre-reg prediction was that frame-coherence would outperform subset-membership by ≥0.10 AUC. **Result: 0/3 HITs, 1 directional.** Frame-coherence works as a discriminator (AUC 0.967 on the H2 primary binary) but does not strictly outperform subset-membership-mean or R²-proximity at n=29 — all three cluster at the AUC ceiling. The schema-redesign route through re-typing `mandatory_features` as frame-evocation is closed. The lens-doc thesis is not load-bearing on this result: pragmatics-as-construction-layer was empirically grounded by silence-manufacture before this test and remains grounded after. What the H2 test was load-bearing on was one specific schema-redesign route; that route closes; the broader lens is unchanged. The follow-up pre-reg #14 (expanded-vintage replication, OTS-stamped same day) tests whether the AUC ceiling is a corpus-size artifact and whether silence appears outside 2016Q1; result by Sunday.
...
**(b) Read FRO with one specific question.** Pull `finregont.com`'s OWL files. Audit whether FRO's hierarchy supports representing *what the regulator has not yet specified* — provisional rules, deliberately-unspecified regions, contested interpretations — vs. only what it has. A first pass against FRO's current shape (see §4 above) finds that FRO types the *interpretation act* (`jc-core:Judicial_Interpretation`, `lkif-proc:created_by`, `fro-leg-ref:DocumentEdition`) but does NOT type provisionality, contested-meaning, or deliberately-unspecified regions — so the gap is real, and the audit question is the place where the project's contribution lives. Confirm or push back on that reading directly against the OWL files; the audit is a first pass, not a verdict.

**(c) Treat SR 26-2 as the design brief.** The Fed's requirement that the model inventory "support effective model risk management at the individual and aggregate levels" — without specifying its structural form — is the closest the regulator has come to opening a window for a governance-ontology artifact. The question to bring is: *what pragmatic slots does that inventory need so that an examiner can ask "is this constraint enforced here, for these products, under this regulation, in this version of the deployed policy?" and get an honest answer?* If the project doesn't supply that shape, a vendor will quietly supply a flat-tabular one first; that's the live competition.
...
**Banking landscape.** FIBO (EDMC, MIT, ~2446 classes, OWL 2 DL aspiration / OWL 2 RL practice). Legal-side: LKIF, LegalRuleML, Akoma Ntoso, ELI/ECLI. FRO as the closest policy-codification prior art (single maintainer; GPL-3.0 + MIT dual-license; interpretation act typed, pragmatic context not). Data-dictionary-adjacent: ISO 20022, BIAN v12.0, ACTUS, MISMO v3.6.2, FFIEC Call Report XBRL, HMDA LAR. Recent: Kumar/Hines/Dickerson (Reg B / fairness-ML gap), SR 26-2 (Fed's aggregate-risk-inventory requirement, structural form unspecified), GDPR Art. 15(1)(h) + CJEU C-203/22 (European explainability anchor), EU AI Act (high-risk classification for credit scoring).

---

## Anchors and uncertain claims

- The lens itself is the project's framing, planted by Tony 2026-05-15 ("Using pragmatics for ontology construction actually makes sense"). The empirical grounding is in `[[project_silence_manufacture_result]]` and `[[project_hmda_trimodal_result]]`, with the H2 frame-evocation result (2026-05-15, also in `[[project_current_anchor]]`) adding a third data point: predictions that assume uniformity fail; indexed claims survive.
...
- FRO's license is declared per-file in the `.ttl` modules as GPL-3.0 + MIT dual-license, though the public page does not state a top-level license.
...
- The SR 26-2 reading above is from a direct read of `references/SR2602a1.pdf` (12-page attachment, April 17, 2026). The letter does not specify the structural form of the model inventory — earlier drafts of this document claimed a "knowledge-graph" expectation that is not in the source text; that claim has been corrected. The regulatory-window framing is sound; the specific operationalization is not yet mandated.
